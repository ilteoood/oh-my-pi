					...
					<Accordion.Heading>
						<Accordion.Trigger>
							<div className="flex items-center gap-2">
								...
								{summary && <span className="text-xs text-muted truncate">{summary}</span>}
								{intent && <span className="text-xs text-muted italic">{intent}</span>}
							</div>
							<Accordion.Indicator />
						</Accordion.Trigger>
					</Accordion.Heading>
					<Accordion.Panel>
						<Accordion.Body>
							<div className="space-y-2 text-sm">
								<div>
									<p className="text-xs text-muted mb-1">{t("tool.arguments")}</p>
									<pre className="bg-surface rounded-lg px-3 py-2 overflow-x-auto text-xs font-mono whitespace-pre-wrap">
								...
								{result !== undefined && (
									<div>
										<p className="text-xs text-muted mb-1">
											{isError ? t("tool.status.error") : t("tool.result")}
										</p>
										<pre
